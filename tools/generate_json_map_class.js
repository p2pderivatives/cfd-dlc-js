    ...
    for (const data in list) {
      ...
      for (const map_key in map_list) {
        ...
        if (has_error_output && (map_data.struct_type.indexOf('ResponseStruct') >= 0)) {
          if (namespace == 'cfd::dlc::js::api') {
            text_array.push(`  InnerErrorResponseStruct error;       //!< error information`);
          } else {
            text_array.push(`  cfd::dlc::js::api::InnerErrorResponseStruct error;   //!< error information`);
          }
        }
...
function convertFile() {
  const fileList = [];
  let cfdBaseDir;
  const cfdPath = `${__dirname}/../external/cfd-dlc-js/`;
  const cfdPath2 = `${__dirname}/../../cfd-dlc-js/`;
  let folderPath = `src/input_json_format/`;
  const outJsonSourceFolderPath = `${__dirname}/../../cfd-dlc-js/src/autogen/`;
  const outJsonHeaderFolderPath = `${__dirname}/../../cfd-dlc-js/src/autogen/`;
  let outStructDirPath = `include/cfddlcjs/`;
  let loadCfdjsIndexFile = '';
  let outTsFolderPath = ``;
  const outStructFileName = `cfddlcjs_struct.h`;
  ...
  fs.readdir(folderPath, (err, files) => {
...

    {
      ...
      const header_str = generateFileHeader(outHeaderFile, outJsonHeaderFolderPath,
          classHeaderList, jsonObjectCommon, `cfddlcjs/${outStructFileName}`);
