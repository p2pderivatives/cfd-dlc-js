import * as cfddlcjs from "../../index.js";
import TestHelper from "./TestHelper";
import * as TestData from "./data/TestData";

const testCase = [
  TestHelper.createTestCase(
    "VerifyCetSignatures",
    cfddlcjs.VerifyCetSignatures,
    {
      cetsHex: [
        TestData.CetHexUnsigned,
        "",
      ],
      localFundPubkey: TestData.LocalFundPubkey,
      remoteFundPubkey: TestData.RemoteFundPubkey,
      fundTxId: TestData.FundTxId,
      fundInputAmount: TestData.FundInputAmount,
      signatures: [
        TestData.CetLocalRawSignature,
        "01e046a58606c9085ff19064b7721c3d8e2f80d5c69f91d384c20e08a9c6b4da5fdca97989e486c62c98de4f7a6e24cd8b1bf7ba296f844a6e61193e9dc646b1",
      ],
      verifyRemote: false,
    },
    {
      valid: true,
    }
  ),
  TestHelper.createTestCase(
    "VerifyCetSignatures invalid signature",
    cfddlcjs.VerifyCetSignatures,
    {
      cetsHex: [
        TestData.CetHexUnsigned,
        "",
      ],
      localFundPubkey: TestData.LocalFundPubkey,
      remoteFundPubkey: TestData.RemoteFundPubkey,
      fundTxId: TestData.FundTxId,
      fundInputAmount: TestData.FundInputAmount,
      signatures: [
        TestData.CetLocalRawSignature,
        "01e046a58616c9085ff19064b7721c3d8e2f80d5c69f91d384c20e08a9c6b4da5fdca97989e486c62c98de4f7a6e24cd8b1bf7ba296f844a6e61193e9dc646b1",
      ],
      verifyRemote: false,
    },
    {
      valid: false,
    }
  ),
];

TestHelper.doTest("VerifyCetSignature", testCase);
