...
CreateDlcTransactionsResponseStruct DlcTransactionsApi::CreateDlcTransactions(
  ...
  auto call_func = [](const CreateDlcTransactionsRequestStruct& request)
    ...
    auto transaction = DlcManager::CreateDlcTransactions(
    ...
    result.fund_tx_hex = transaction.fund_transaction.GetHex();
    result.refund_tx_hex = transaction.refund_transaction.GetHex();
    for (auto tx : transaction.cets) {
      result.closing_cets_hex.push_back(tx.GetHex());
    }
    return result;
  };
  CreateDlcTransactionsResponseStruct result;
